...
$DGAS_LOCATION/bin/glite-dgas-hlr-query -Q resourceAggregate -j "%start%"
$DGAS_LOCATION/bin/glite-dgas-hlr-query -Q resourceAggregate -j "%start%" | sed -e 's/\s//g' | cut -d "|" -f 1 | grep "1975-04-27"
