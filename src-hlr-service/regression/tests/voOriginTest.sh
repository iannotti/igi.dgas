...
$GLITE_LOCATION/bin/glite-dgas-hlr-query -Q resourceAggregate -A "count(dgJobId)" -j "%voorigin%" -G "voOrigin"
$GLITE_LOCATION/bin/glite-dgas-hlr-query -Q resourceAggregate -A "count(dgJobId)" -j "%voorigin%" -G "voOrigin" | grep fqan
