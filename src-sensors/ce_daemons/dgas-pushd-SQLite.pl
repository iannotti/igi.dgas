...
my $dbh = DBI->connect("dbi:SQLite:$configValues{dgasDB}")
  || &printLog( 1, "Error opening database." ) && &Exit();
...
while ($keepGoing)
{
	...
	&printLog( 3,
"Waiting for $configValues{dgasDB}:commands table to be created by urcolelctor."
	);
	sleep 1;
}
...
" SELECT key,transport,composer,arguments,producer,commandStatus FROM commands ORDER by commandStatus desc limit $limit"
	  );
	$dbh->commit;
	my $numOfRecords = 0;
	if (@$commandKeys)
...
"Spawning new process:$threadNumber on record: $key[$threadNumber]"
		...
		foreach (@childs)
		{
			...
			&printLog( 9,
"thread pid:$_,ThreadNumber:$commandThreadNumber[$_],status:$result"
			);
			...
			else
			{
				...
				&pushToQueue( $key[$threadNumber],
					$commandStatus[$threadNumber] );
			}
		}
	}
	for (
