...
my $DEF_LDIF_VALIDITY = 86400; # assume the GLUE attributes taken from the LDIF didn't change within the last day.
...
my %configValues = (
		    ...
		    lrmsType              =>  "pbs",
     		    ...
     		    collectorLogFileName =>  $dgasLocation . "/var/log/dgas/dgas_urcollector.log",
...
my $dbh = DBI->connect( "dbi:SQLite:$configValues{dgasDB}" ) || die "Cannot connect: $DBI::errstr";
...
my $sqlStatement = "CREATE TABLE IF NOT EXISTS commands (key INTEGER PRIMARY KEY, transport TEXT KEY, composer TEXT, arguments TEXT, producer TEXT, recordDate TEXT, lrmsId TEXT, commandStatus INT)";
...
sub callAtmClient 
{
	...
	$cmd .= " \"si2k=$si2k\"";
	&printLog ( 8, "Got 'GlueHostApplicationSoftwareRunTimeEnvironment: SI00MeanPerCPU' from UR file: $si2k");
	&printLog ( 9, "(note: Taking mean SI00, ignoring GlueHostBenchmarkSI00)");
    }
	...
	&printLog ( 8, "Got 'GlueHostBenchmarkSI00' from UR file: $urGridInfo{GlueHostBenchmarkSI00}");
    }
	...
	$cmd .= " \"sf2k=$sf2k\"";
	&printLog ( 8, "Got 'GlueHostApplicationSoftwareRunTimeEnvironment: SF00MeanPerCPU' from UR file: $sf2k");
	&printLog ( 9, "(note: Taking mean SF00, ignoring GlueHostBenchmarkSF00)");
    }
...
sub processLrmsLogFile {
	...
	else 
	{
	    ...
	    {
		&printLog (2,"Error: could not create UR for job $targetJobId with LRMS event time: $lrmsEventTimeString!");
...
=$jhClusterId. Stopping iteration ...\n");
                    ...
                    if ($tmpBuffTstamp < 0) {
                        ...
                        # completed:                        
			&printLog (7,"Very first job to process. Writing temporary buffer for backward processing until completing first iteration!\n");
                    ...
                    if ($firstJobId) {
                        ...
                        &printLog (6, "Most recent job to process: $jhClusterId; CompletionDate: $jhCompletionDate\n");
                    }
...
sub getGLUEAttributesFromLdif {
    ...
    if ($modTStamp == 0) {
	&printLog (5,"Warning: No GLUE attributes added to UR (reason: no valid entries in LDIF file(s))!");
	return 0;
    }
    else {
...
sub getLrmsTargetJobId_sge {
	...
	{
		&printLog (3, "Warning: SGE submission_time have a wrong format in line: $_[0]; skipping job.");
	}	
